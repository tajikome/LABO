(defun c:SSD (/ *error* check-layer oldCmdecho oldLayer oldDimlayer oldDimexo res dColor overrideColor pt1 pt2 ptLoc curPt loop set-prop history dimEnt lastOp get-dim-color calc-ptLoc color-name offDist)
  ...
  (if (and (setq pt1 (getpoint "\n1点目: "))
    ...
    (progn
      ...
      (setq history (list (list pt1 pt2 dimEnt)))
      ...
      (while loop
        ...
        (cond
          ...
          ((= (type res) 'LIST)
            ...
            (setq history (cons (list curPt res dimEnt) history))
            (setq curPt res)
          )
          (T (setq loop nil))
        )
